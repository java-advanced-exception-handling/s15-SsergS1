...
@WebMvcTest({TaskController.class, UserControllerAdvice.class})
public class TaskControllerTest {
    ...
    @Test
    void read_ShouldReturn404_WhenNotFound() throws Exception {
        when(taskService.readById(100L)).thenThrow(new EntityNotFoundException("Task with id 100 not found"));

        mockMvc.perform(get("/tasks/100/update/todos/1"))
                .andExpect(status().isNotFound())
                .andExpect(view().name("error/404"))
                .andExpect(model().attribute("message", "Task with id 100 not found"));
    }
}
